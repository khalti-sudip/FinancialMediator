...
def execute_provider_request(request_data, provider_config):
    ...
    while retry_count <= max_retries:
        try:
            ...
            logger.warning(f"Request timeout to provider {provider_config.system_name}, attempt {retry_count + 1}/{max_retries + 1}")
            retry_count += 1
            if retry_count <= max_retries:
                # Exponential backoff
                time.sleep(2 ** retry_count)
            else:
                logger.error(f"Max retries reached for provider {provider_config.system_name}")
                return {"error": "Request timeout after retries"}, 504
        
        except requests.exceptions.RequestException as e:
            logger.error(f"Request error to provider {provider_config.system_name}: {str(e)}")
...
def create_provider_config():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"error": "Invalid provider configuration", "details": err.messages}), 400
        
        system_name = data['system_name']
        
        # Check if provider already exists
        existing_provider = SystemConfig.query.filter_by(system_name=system_name).first()
...
def get_provider_config(provider_name):
    ...
    current_user = get_jwt_identity()
    logger.info(f"Retrieving provider {provider_name} configuration by user {current_user}")
...
def update_provider_config(provider_name):
    ...
    current_user = get_jwt_identity()
    logger.info(f"Updating provider {provider_name} configuration by user {current_user}")
...
def test_provider_connection(provider_name):
    ...
    current_user = get_jwt_identity()
    logger.info(f"Testing connection to provider {provider_name} by user {current_user}")
    ...
    try:
        ...
        if status_code >= 200 and status_code < 300:
            ...
            logger.warning(f"Test connection to provider {provider_name} failed with status {status_code}")
            return jsonify({
