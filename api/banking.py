...
def execute_banking_request(request_data, banking_config):
    ...
    if not api_key and banking_config.auth_type != "none":
        logger.error(
            f"API key not found for banking system {banking_config.system_name}"
        )
    ...
    while retry_count <= max_retries:
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Banking system response is not JSON: {response.text[:100]}"
                )
            ...
            logger.warning(
                f"Request timeout to banking system {banking_config.system_name}, attempt {retry_count + 1}/{max_retries + 1}"
            )
            retry_count += 1
            if retry_count <= max_retries:
                ...
                logger.error(
                    f"Max retries reached for banking system {banking_config.system_name}"
                )
                return {"error": "Request timeout after retries"}, 504

        except requests.exceptions.RequestException as e:
            logger.error(
                f"Request error to banking system {banking_config.system_name}: {str(e)}"
            )
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "error": "Invalid banking system configuration",
        ...
        if existing_system:
            ...
            return (
                jsonify({"error": f"Banking system {system_name} already exists"}),
                409,
            )
...
def get_banking_config(system_name):
    ...
    current_user = get_jwt_identity()
    logger.info(
        f"Retrieving banking system {system_name} configuration by user {current_user}"
    )

    try:
        banking_config = SystemConfig.query.filter_by(
            system_name=system_name, system_type="banking_system"
        ).first()
...
def get_all_banking_systems():
    ...
    try:
        banking_systems = SystemConfig.query.filter_by(
            system_type="banking_system"
        ).all()
...
def update_banking_config(system_name):
    ...
    current_user = get_jwt_identity()
    logger.info(
        f"Updating banking system {system_name} configuration by user {current_user}"
    )
...
def test_banking_connection(system_name):
    ...
    current_user = get_jwt_identity()
    logger.info(
        f"Testing connection to banking system {system_name} by user {current_user}"
    )
    ...
    try:
        ...
        if status_code >= 200 and status_code < 300:
            ...
            logger.warning(
                f"Test connection to banking system {system_name} failed with status {status_code}"
            )
            return (
                jsonify(
                    {
