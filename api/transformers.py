...
def transform_request(request_data, source_system, target_system):
    ...
    """
    logger.debug(f"Transforming request from {source_system.system_name} to {target_system.system_name}")
    ...
    if target_system.system_type == 'financial_provider':
        transformed_data = transform_to_provider_format(transformed_data, transaction_type)
    elif target_system.system_type == 'banking_system':
        transformed_data = transform_to_banking_format(transformed_data, transaction_type)
...
def transform_response(response_data, source_system, target_system):
    ...
    """
    logger.debug(f"Transforming response from {source_system.system_name} to {target_system.system_name}")
    ...
    # Apply specific transformations based on source and target systems
    if source_system.system_type == 'financial_provider' and target_system.system_type == 'banking_system':
        transformed_data = transform_provider_to_banking_response(transformed_data)
    elif source_system.system_type == 'banking_system' and target_system.system_type == 'financial_provider':
...
def transform_to_provider_format(data, transaction_type):
    ...
    if transaction_type == 'payment':
        ...
        transformed['payload'] = {
            'paymentRequest': {
                ...
                'beneficiary': {
                    ...
                    'accountNumber': payment.get('beneficiary', {}).get('account_number', ''),
                    'bankDetails': {
                ...
                'executionDate': payment.get('execution_date', datetime.date.today().isoformat())
            }
        }
        ...
        transformed['payload'] = {
            'balanceRequest': {
                'accountNumber': transformed.get('payload', {}).get('account_number', ''),
                'includePending': transformed.get('payload', {}).get('include_pending', False)
            }
        }
    ...
    if transaction_type == 'payment':
        ...
        transformed['payload'] = {
            'inquiry': {
                ...
                'accountNumber': transformed.get('payload', {}).get('account_number', ''),
                'options': {
                    'includePendingTransactions': transformed.get('payload', {}).get('include_pending', False)
                }
            }
        }
