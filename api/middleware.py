...
def process_transaction():
    ...
    try:
        ...
        if validation_errors:
            ...
            return jsonify({'error': 'Transaction validation failed', 'details': validation_errors}), 400
        
        # Check if we have the necessary system configurations
        source_system = SystemConfig.query.filter_by(
            system_name=transaction_data['source_system'],
            is_active=True
        ).first()
        
        target_system = SystemConfig.query.filter_by(
            system_name=transaction_data['target_system'],
            is_active=True
        ).first()
        
        if not source_system or not target_system:
            logger.error(f"System configuration not found or inactive for source: {transaction_data['source_system']} or target: {transaction_data['target_system']}")
        ...
        if transaction_data.get('cacheable', False):
            ...
            if cached_response:
                logger.info(f"Returning cached response for transaction: {transaction_id}")
        ...
        transformed_request = transform_request(transaction_data, source_system, target_system)
        
        # Process based on target system type
        if target_system.system_type == 'financial_provider':
            result, status_code = execute_provider_request(transformed_request, target_system)
        elif target_system.system_type == 'banking_system':
            result, status_code = execute_banking_request(transformed_request, target_system)
        else:
            logger.error(f"Unsupported target system type: {target_system.system_type}")
            transaction.status = 'failed'
            transaction.error_message = f"Unsupported target system type: {target_system.system_type}"
            db.session.commit()
        ...
        if status_code >= 200 and status_code < 300:
            ...
            transformed_response = transform_response(result, target_system, source_system)
            ...
            logger.error(f"Transaction failed: {transaction_id}, Error: {result.get('error', 'Unknown error')}")
            return jsonify({'error': 'Transaction processing failed', 'details': result}), status_code
...
def get_transaction_status(transaction_id):
    ...
    current_user = get_jwt_identity()
    logger.info(f"Retrieving transaction status for {transaction_id} by user {current_user}")
        ...
        if transaction_id:
            ...
            if transaction:
                ...
                if event_type == 'transaction_completed':
                    ...
                    transaction.status = 'failed'
                    transaction.error_message = webhook_data.get('error_message', 'No error details provided')
